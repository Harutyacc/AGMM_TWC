...
%% 生成符合参数的真实数据
% load('RealEnv_Data\Direc915Test.mat');  % test_estimat_1
% load('RealEnv_Data\Person915Test.mat'); % test_estimat_2
...
for n = 1:600                                                          % 测试了600帧的数据
    ...
    for j=1:real_Iterations
        ...
        % % 构建KLD（积分） 
        % for i_rect_x = 1:length(x_hist)
        %     for i_rect_y = 1:length(y_hist)
        %         % 定义被积函数
        %         f_real = PDF_d_int(i_rect_x, i_rect_y);
        %         f_mixture = @(x, y) sum(arrayfun(@(idx) w(j+1,idx) * mvnpdf([x y], u(idx,:), s(:,:,idx)), 1:k));
        %         % 确定积分上下限
        %         x_min = x_edges(i_rect_x);
        %         x_max = x_edges(i_rect_x + 1);
        %         y_min = y_edges(i_rect_y);
        %         y_max = y_edges(i_rect_y + 1);
        %         % 计算积分
        %         P_d = PDF_d_int*(x_max - x_min) * (y_max - y_min);
        %         P_t = integral2(f_mixture, x_min, x_max, y_min, y_max);
        %         % 计算KLD
        %         KLD_d(j+1) = KLD_d(j+1) +  P_d*log(P_d/P_t);
        %         for k_tmp = 1:k
        %             f_single = @(x, y) w(j+1, k_tmp) * mvnpdf([x y], u(k_tmp,:), s(:,:,k_tmp));
        %             P_s = integral2(f_single, x_min, x_max, y_min, y_max);
        %             KLD_t(j+1, k_tmp) = KLD_t(j+1, k_tmp) + P_s*log(P_s/P_d);
        %         end
        %     end
        % end
        ...
        % Subplot 1: Scatter plot of data and cluster centers
        subplot(2,2,1);
        hold off
        scatter(data(:,1), data(:,2));
        hold on;
        scatter(u(:,1), u(:,2), 200, 'MarkerFaceColor', 'r');
        text(u(:,1), u(:,2), num2cell(1:k));
        xlabel('In-phase');
        ylabel('Quadrature');
        title('Data and Cluster Centers');

        % Subplot 2: Bar plot of kernel norms
        subplot(2,2,2);
        bar(pr(j+1,:)');
        xlabel('Kernels');
        ...
        % Subplot 3: 3D Scatter plot of density, weight, and gap
        subplot(2,2,3);
        z=[];
        for i=1:k
        ...
        ylabel('Weight');
        zlabel('Gap');
        title('Density, Weight, and Gap');

        % Subplot 4: Plot of log likelihood and KLD
        subplot(2,2,4);
        x_l = 2:length(l);
        ...
        % Adjust subplot positions for better appearance
